...
Ten tools cover the full loop: read/write the draft (`get_checklist_status`, `update_card_field`, `assemble_card`), enumerate and add test openings (`list_scenarios`, `add_scenario`), and drive + inspect playtests (`run_scenario`, `get_playtest_context`, `commit_playtest_round`, `get_transcript`, `compare_scenarios`). `get_checklist_status` is also the front door to the *writing* phase: it returns nine completion aspects (seven richness aspects, the MVU decision, and the card-beautify direction), each annotated with a decision axis and an output target, so an Agent handed nothing but a few sentences of world/character/backstory can walk a creator through the gaps — proposing two or three concrete options per aspect that diverge along that axis — instead of guessing a whole card in one shot. Because the tools are typed and self-describing (`inputSchema` + a `description` written for an Agent audience, not a human one — available in Traditional Chinese and English, following the page's 中/EN toggle so the Agent reads whichever language the creator is working in), an Agent can discover the right call sequence from the tool metadata alone, with no external documentation or prompt scaffolding required.
